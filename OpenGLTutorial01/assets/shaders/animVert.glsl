...
uniform mat4 prevBones[MAXNUMBEROFBONES];
...
void main()
{
	...
	mat4 PrevBoneTransform = prevBones[ int(aBoneIDs[0]) ] * aBoneWeights[0];
	PrevBoneTransform += prevBones[ int(aBoneIDs[1]) ] * aBoneWeights[1];
	PrevBoneTransform += prevBones[ int(aBoneIDs[2]) ] * aBoneWeights[2];
	PrevBoneTransform += prevBones[ int(aBoneIDs[3]) ] * aBoneWeights[3];
	...
	ClipSpacePos0 = matMVP * vertPosition;
	vec4 PrevVertPos = PrevBoneTransform * vec4(aPos, 1.0);
	PrevClipSpacePos0 = matMVP * PrevVertPos;
}		