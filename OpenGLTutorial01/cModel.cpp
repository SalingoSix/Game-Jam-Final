...
cModel::cModel(std::string path, std::vector<glm::mat4> modelMatrices, GLuint texID)
{
	vecModels = modelMatrices;
	this->texID = texID;
	loadModel(path);
}
...
cMesh cModel::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	if(vecModels.size() == 0)
		...
		return cMesh(theVertices, theIndices, theTextures, vecModels, texID);
}
